    ...
    <div className={`w-full ${className}`}>
      <div ref={dropdownRef} className="relative w-full min-w-0">
        <div className="relative w-full bg-white/10 text-[#fff] rounded-lg border border-[#555] flex items-center lg:h-18 h-13 overflow-hidden focus-within:ring-[2px] focus-within:ring-[#666] focus-within:border-[#666] lg:placeholder:text-[16px] placeholder:text-[12px]">
          ...
          >
            <span className="text-lg text-[#888]">{selectedCountry.flag}</span>
              ...
              {filteredCountries.length > 0 ? (
                filteredCountries.map((country, index) => (
                  <button
                    ...
                    className="w-full px-4 py-3 text-left text-sm hover:bg-[#333] transition-colors flex items-center gap-3 border-b border-[#555] last:border-b-0"
                    style={{
                  ...
                  >
                    ...
                    <span className="font-medium text-[#fff] ">{country.code}</span>
                    <span className="text-[#fff]">{country.country}</span>
                  </button>
                ))
              ) : (
